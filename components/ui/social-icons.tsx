...
const socials = [
  ...
  {
    name: "Facebook",
    href: "https://facebook.com/euroceram2002",
    icon: (
      <svg viewBox="0 0 24 24" fill="currentColor" className="size-[18px]">
  ...
  {
    name: "Instagram",
    href: "https://www.instagram.com/euroceram_2002?igsh=MXA1djUxYzFubWY1aA==",
    icon: (
      <svg viewBox="0 0 24 24" fill="currentColor" className="size-[18px]">
...
export function SocialIcons({ size = 'sm' }: { size?: 'sm' | 'lg' }) {
  const [hoveredIndex, setHoveredIndex] = useState<number | null>(null)
  const btnSize = size === 'lg' ? 'size-16' : 'size-10'
  const iconScale = size === 'lg' ? 'scale-[1.6]' : ''

  return (
    <div className="relative w-fit flex items-center gap-1 px-2 py-2 rounded-2xl bg-neutral-950 border border-white/[0.08]">
      ...
      {socials.map((social, index) => (
        <a
          ...
          className={`group relative flex items-center justify-center ${btnSize} rounded-xl transition-colors duration-200`}
        ...
        >
          ...
          <span
            className={`relative z-10 transition-all duration-300 ease-out ${iconScale} ${
